...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateC0C1(char *o_pcBuf,int i_iMaxBufLen)
{
    int iRet = -1;
    int iDataLen = 0;
    char* pcBuf = NULL;

    if(i_iMaxBufLen < RTMP_C0_LEN+RTMP_C1_LEN || NULL == o_pcBuf )
    {
    ...
    memset(&o_pcBuf[iDataLen],0,4);//memcpy(&o_pcBuf[iDataLen],i_pcC1,4);//SRS// s1 time2 copy from c1
    iDataLen+=4;
    iRet = RandomGenerate(&o_pcBuf[iDataLen],RTMP_C1_LEN - 8);
    if(iRet < 0)
    {
        RTMP_LOGE("RandomGenerate err %d %d\r\n",iDataLen,RTMP_C1_LEN);
        return iRet;
    }
    iDataLen += RTMP_C1_LEN - 8;
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateC2(char *i_pcS1,int i_iS1Len,char *o_pcBuf,int i_iMaxBufLen)
{
    int iRet = -1;
    int iDataLen = 0;
    char* pcBuf = NULL;

    if(i_iMaxBufLen < RTMP_C2_LEN || NULL == i_pcS1 ||i_iS1Len != RTMP_S1_LEN)
    {
        RTMP_LOGE("CreateC2 err %d %d\r\n",i_iMaxBufLen,i_iS1Len);
        return iRet;
    }

    memcpy(&o_pcBuf[iDataLen], i_pcS1, RTMP_S1_LEN);// if c1 specified, copy c1 to s2.
    iDataLen+=RTMP_S1_LEN;//time2（4 bytes）：本字段必须包含时间戳，取值为接收对端发送过来的握手包的时刻
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateCmdConnect(char *i_strAppName,char *i_strTcURL,unsigned char* o_pbBuf, int i_iBufMaxLen)
{
    int iLen = 0;
    unsigned char * pbEnd = o_pbBuf + i_iBufMaxLen;
    unsigned char * pbOffset = o_pbBuf;
    const char* strCommand = "connect";
    const char *strStatusLevel = NULL;
    const char * strCode = NULL;

    if(NULL == i_strAppName || NULL == i_strTcURL || NULL == o_pbBuf)
    {
    ...
    pbOffset = AMFWriteString(pbOffset, pbEnd, strCommand, strlen(strCommand));// Command Name
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, 1);// Transaction ID,协议规定总是1
    
    pbOffset = AMFWriteObject(pbOffset, pbEnd);
    pbOffset = AMFWriteNamedString(pbOffset, pbEnd, "app", strlen("app"), i_strAppName, strlen(i_strAppName));
    pbOffset = AMFWriteNamedString(pbOffset, pbEnd, "type", strlen("type"), "nonprivate", strlen("nonprivate"));
    pbOffset = AMFWriteNamedString(pbOffset, pbEnd, "flashVer", strlen("flashVer"), "FMLE/3.0 (compatible; Lavf60.14.100)", strlen("FMLE/3.0 (compatible; Lavf60.14.100)"));
    pbOffset = AMFWriteNamedString(pbOffset, pbEnd, "tcUrl", strlen("tcUrl"),i_strTcURL, strlen(i_strTcURL));
    pbOffset = AMFWriteObjectEnd(pbOffset, pbEnd);
    iLen = pbOffset - o_pbBuf;
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateCmdCreateStream(unsigned char* o_pbBuf, int i_iBufMaxLen)
{
    int iLen = 0;
    unsigned char * pbEnd = o_pbBuf + i_iBufMaxLen;
    unsigned char * pbOffset = o_pbBuf;
    const char* strCommand = "createStream";

    if(NULL == o_pbBuf)
    {
        RTMP_LOGE("CreateCmdCreateStream err %d \r\n",i_iBufMaxLen);
        return -1;
    }

    pbOffset = AMFWriteString(pbOffset, pbEnd, strCommand, strlen(strCommand));// Command Name
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, 4);// Transaction ID,协议规定总是4
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateCmdPlay(char *i_strStreamName,unsigned char* o_pbBuf, int i_iBufMaxLen)
{
    int iLen = 0;
    unsigned char * pbEnd = o_pbBuf + i_iBufMaxLen;
    unsigned char * pbOffset = o_pbBuf;
    const char* strCommand = "play";
    double dlStart = -2; // the start time in seconds, [default] -2-live/vod, -1-live only, >=0-seek position
    double dlDuration; // duration of playback in seconds, [default] -1-live/record ends, 0-single frame, >0-play duration
    
    if(NULL == i_strStreamName ||NULL == o_pbBuf)
    {
        RTMP_LOGE("CreateCmdCreateStream err %d \r\n",i_iBufMaxLen);
        return -1;
    }
    ...
    pbOffset = AMFWriteString(pbOffset, pbEnd, strCommand, strlen(strCommand));// Command Name
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, 0);// Transaction ID,协议规定总是0
    
    pbOffset = AMFWriteNull(pbOffset, pbEnd);// command object
    pbOffset = AMFWriteString(pbOffset, pbEnd, i_strStreamName, strlen(i_strStreamName));
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, dlStart);
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateCmdPublish(char *i_strStreamName,unsigned char* o_pbBuf, int i_iBufMaxLen)
{
    int iLen = 0;
    unsigned char * pbEnd = o_pbBuf + i_iBufMaxLen;
    unsigned char * pbOffset = o_pbBuf;
    const char* strCommand = "publish";
    const char* strStreamType = "live";
    ...
    pbOffset = AMFWriteNull(pbOffset, pbEnd);// command object
    pbOffset = AMFWriteString(pbOffset, pbEnd, i_strStreamName, strlen(i_strStreamName));
    pbOffset = AMFWriteString(pbOffset, pbEnd, strStreamType, strlen(strStreamType));
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpPack::CreateCmdDeleteStream(unsigned char* o_pbBuf, int i_iBufMaxLen)
{
    int iLen = 0;
    unsigned char * pbEnd = o_pbBuf + i_iBufMaxLen;
    unsigned char * pbOffset = o_pbBuf;
    const char* strCommand = "deleteStream";
    const char *strStatusLevel = NULL;
    const char * strCode = NULL;

    if(NULL == o_pbBuf)
    {
        RTMP_LOGE("CreateCmdDeleteStream err %d \r\n",i_iBufMaxLen);
        return -1;
    }

    pbOffset = AMFWriteString(pbOffset, pbEnd, strCommand, strlen(strCommand));// Command Name
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, 7);
    pbOffset = AMFWriteNull(pbOffset, pbEnd);
    pbOffset = AMFWriteDouble(pbOffset, pbEnd, 1);
