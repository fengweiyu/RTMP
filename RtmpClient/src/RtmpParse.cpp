...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpParse::ParseCmd_result(unsigned char *i_pbBuf,int i_iBufLen,T_RtmpResultContent *o_ptRtmpResultContent)
{
    int iRet = -1;
    T_RtmpResultContent tRtmpResultContent;
    struct amf_object_item_t atItems[2];
    struct amf_object_item_t atCommands1[2];
    struct amf_object_item_t atCommands2[4];

    
    if(NULL == i_pbBuf || NULL == o_ptRtmpResultContent )
    {
        RTMP_LOGE("ParseCmdConnect err %d \r\n",i_iBufLen);
        return iRet;
    }
    ...
    tRtmpResultContent.dlEncoding = (double)RTMP_CMD_ENCODING_AMF_0;
    AMF_OBJECT_ITEM_VALUE(atCommands1[0], AMF_STRING, "fmsVer", tRtmpResultContent.strFmsVer, sizeof(tRtmpResultContent.strFmsVer));
    AMF_OBJECT_ITEM_VALUE(atCommands1[1], AMF_NUMBER, "capabilities", &tRtmpResultContent.dlCapabilities, 8);
    AMF_OBJECT_ITEM_VALUE(atItems[0], AMF_OBJECT, "command", atCommands1, sizeof(atCommands1) / sizeof(struct amf_object_item_t));

    AMF_OBJECT_ITEM_VALUE(atCommands2[0], AMF_STRING, "level", tRtmpResultContent.strLevel, sizeof(tRtmpResultContent.strLevel));
    AMF_OBJECT_ITEM_VALUE(atCommands2[1], AMF_STRING, "code", tRtmpResultContent.strCode, sizeof(tRtmpResultContent.strCode));
    AMF_OBJECT_ITEM_VALUE(atCommands2[2], AMF_STRING, "description", &tRtmpResultContent.strDescription, 8);
    AMF_OBJECT_ITEM_VALUE(atCommands2[3], AMF_NUMBER, "objectEncoding", &tRtmpResultContent.dlEncoding, 8);
    AMF_OBJECT_ITEM_VALUE(atItems[1], AMF_OBJECT, "command", atCommands2, sizeof(atCommands2) / sizeof(struct amf_object_item_t));

    iRet = amf_read_items(i_pbBuf, i_pbBuf + i_iBufLen, atItems, sizeof(atItems) / sizeof(struct amf_object_item_t)) ? 0 : -1;
    
    memcpy(o_ptRtmpResultContent, &tRtmpResultContent, sizeof(T_RtmpResultContent));
    return iRet;
}

/*****************************************************************************
-Fuction        : ParseCmdOnStatus
-Description    : 
-Input          : 
-Output         : 
-Return         : Version
...
* 2023/09/21      V1.0.0         Yu Weifeng       Created
******************************************************************************/
int RtmpParse::ParseCmdOnStatus(unsigned char *i_pbBuf,int i_iBufLen,T_RtmpOnStatusContent *o_ptRtmpOnStatusContent)
{
    int iRet = -1;
    T_RtmpOnStatusContent tRtmpOnStatusContent;
    struct amf_object_item_t atItems[2];
    struct amf_object_item_t atCommands[3];

    
    if(NULL == i_pbBuf || NULL == o_ptRtmpOnStatusContent )
    {
        RTMP_LOGE("ParseCmdConnect err %d \r\n",i_iBufLen);
        return iRet;
    }
    ...
    AMF_OBJECT_ITEM_VALUE(atCommands[0], AMF_STRING, "level", tRtmpOnStatusContent.strLevel, sizeof(tRtmpOnStatusContent.strLevel));
    AMF_OBJECT_ITEM_VALUE(atCommands[1], AMF_STRING, "code", tRtmpOnStatusContent.strCode, sizeof(tRtmpOnStatusContent.strCode));
    AMF_OBJECT_ITEM_VALUE(atCommands[2], AMF_NUMBER, "description", &tRtmpOnStatusContent.strDescription, 8);
    AMF_OBJECT_ITEM_VALUE(atItems[1], AMF_OBJECT, "command", atCommands, sizeof(atCommands) / sizeof(struct amf_object_item_t));

    iRet = amf_read_items(i_pbBuf, i_pbBuf + i_iBufLen, atItems, sizeof(atItems) / sizeof(struct amf_object_item_t)) ? 0 : -1;
    
    memcpy(o_ptRtmpOnStatusContent, &tRtmpOnStatusContent, sizeof(T_RtmpOnStatusContent));
    return iRet;
}
