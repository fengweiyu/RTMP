...
typedef struct RtmpResultContent
{
    char strFmsVer[32];
...
typedef struct RtmpOnStatusContent
{
    char strCode[64];
    char strLevel[64];
    char strDescription[64];
}T_RtmpOnStatusContent;
...
public:
    ...
    int ParseCmd_result(unsigned char *i_pbBuf,int i_iBufLen,T_RtmpResultContent *o_ptRtmpResultContent);
    int ParseCmdOnStatus(unsigned char *i_pbBuf,int i_iBufLen,T_RtmpOnStatusContent *o_ptRtmpOnStatusContent);
